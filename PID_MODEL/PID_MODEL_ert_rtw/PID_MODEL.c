 ...
 * C/C++ source code generated on : Thu Nov  9 21:28:30 2023
 *
...
void PID_MODEL_step(void)
{
  ...
  switch ((int32_T)rtU.status) {
   ...
   case 2:
    ...
    /* Chart: '<S3>/Chart1' */
    if (rtDW.is_active_c2_PID_MODEL == 0U) {
      rtDW.is_active_c2_PID_MODEL = 1U;
      rtDW.is_c2_PID_MODEL = IN_IDLE;
    } else {
      switch (rtDW.is_c2_PID_MODEL) {
       case IN_ADD:
        if (rtb_Sum3 <= -5000.0) {
          rtDW.is_c2_PID_MODEL = IN_SUB;
          rtDW.u2 = -1.0;
    ...
    /* Chart: '<S3>/Chart2' */
    if (rtDW.is_active_c1_PID_MODEL == 0U) {
      rtDW.is_active_c1_PID_MODEL = 1U;
      rtDW.is_c1_PID_MODEL = IN_DB;
    } else if (rtDW.is_c1_PID_MODEL == IN_DB) {
      if (rtP.DeadBand <= 0.0) {
        rtDW.is_c1_PID_MODEL = IN_NDB;
      }
